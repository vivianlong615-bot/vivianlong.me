/**
 * Recolor the entire desk (top + legs/drawers/sides) → black on baked_environment.jpg.
 * Uses desk mesh faces from environment.glb for precise UV masking.
 *
 * Run: node scripts/patch-desk-blue.js
 */
...
function buildDeskMask(json, bin) {
...
async function main() {
    ...
    const mask = buildDeskMask(json, bin);
    let masked = 0;
    ...
    for (let y = 0; y < ATLAS; y++) {
        for (let x = 0; x < ATLAS; x++) {
            ...
            const i = (y * ATLAS + x) * 4;
            const next = toBlack(data.data[i], data.data[i + 1], data.data[i + 2]);
            data.data[i] = next.r;
    ...
    console.log(`Desk (top + body) recolored → black — ${masked} px mask, ${painted} px painted`);
